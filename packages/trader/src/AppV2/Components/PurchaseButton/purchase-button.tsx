...
const PurchaseButton = observer(() => {
    ...
    const prev_has_open = usePrevious(test_has_open);
    ...
    React.useEffect(() => {
        const is_animated =
            (!prev_has_open && has_open_accu_contract) || (prev_has_open && !has_open_accu_contract && is_accumulator);
    ...
    return (
        <React.Fragment>
            ...
            <CSSTransition
                in={is_sell_button_visible}
                // timeout={450}
                //should be 0 if switching from non-acc
                timeout={animation_duration}
                classNames='slide'
                key='sell-button'
                unmountOnExit
