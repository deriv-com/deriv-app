...
const DeactivateAccountReason = ({ onBackClick, mt5_login_list, client_accounts, dxtrade_accounts_list }) => {
    ...
    const [is_account_deactivated, setAccountDeactivated] = useState(false);
    const [is_loading, setLoading] = useState(false);
    const [is_modal_open, setModalVisibility] = useState(false);
    const [which_modal_should_render, setModalType] = useState();
    const [reason, setReason] = useState(null);
    const [is_checkbox_disabled, setCheckboxDisabled] = useState(false);
    const [total_checkbox_checked, setTotalCheckboxChecked] = useState(0);
    const [remaining_characters, setRemainingCharacters] = useState(character_limit_no);
    const [total_accumulated_characters, setTotalAccumulatedCharacters] = useState(0);
