...
import { DesktopWrapper, Dialog, MobileOrTabletWrapper, Tabs } from '@deriv/components';
...
const AppWrapper = observer(() => {
    ...
    const { ui } = useStore();
    const { url_hashed_values, is_mobile_or_tablet } = ui;
    ...
    React.useEffect(() => {
        ...
        if (init_render.current) {
            ...
            if (is_mobile_or_tablet) handleTabChange(Number(active_hash_tab));
    ...
    return (
        <React.Fragment>
            <div className='main'>
                <div
                    className={classNames('main__container', {
                        'main__container--active': active_tour && active_tab === DASHBOARD && is_mobile_or_tablet,
                    })}
                >
                    <Tabs
            ...
            <MobileOrTabletWrapper>{!is_open && <RunPanel />}</MobileOrTabletWrapper>
            <Dialog
