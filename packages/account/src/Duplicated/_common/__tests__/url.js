...
function runTests(url) {
    ...
    describe('.paramsHash()', () => {
        ...
        it('returns empty object when there is no query string', () => {
            expect(Url.paramsHash(url_no_qs)).to.be.an('Object').and.to.deep.equal({});
    ...
    describe('.urlFor()', () => {
        ...
        it('handles all valid characters', () => {
            expect(Url.urlFor('metatrader/comparison-4_vs_5')).to.eq(`${website_url}metatrader/comparison-4_vs_5.html`);
        });
    });
