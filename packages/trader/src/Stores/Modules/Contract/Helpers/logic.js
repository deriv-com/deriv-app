...
export const getDisplayStatus = contract_info => {
    ...
    if (isContractWaiting(contract_info)) {
        status = 'waiting';
    }
    return status;
};
...
export const isContractWaiting = contract_info =>
    !!(
        /settlement|This market is presently closed/.test(contract_info.validation_error) &&
