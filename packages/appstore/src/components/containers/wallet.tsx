...
import { useModal, useToggle } from '@deriv/hooks';
import useTestModal, {
    TestModalForHook,
    TestModalWithUsualProps,
} from '../../wallets/components/modals/test-modal/test-modal';
import './wallet.scss';
import { useModalContext } from '../../wallets/context/modal-context';
...
const Wallet = ({ wallet_account }: TWallet) => {
    ...
    const { show_test_modal, setShowTestModal } = useModalContext(); // from context
    ...
    return (
        <div ref={headerRef} className={classNames('wallet', { wallet__demo: is_demo })}>
            ...
            <button onClick={() => setShowTestModal(!show_test_modal)}>toggle from context</button>
