...
const RouteWithSubRoutes = route => {
    const validateRoute = pathname => {
        if (pathname.startsWith(routes.cashier) && !pathname.startsWith(routes.cashier_p2p) && !!route.routes) {
            return route.path === pathname || !!route?.routes.find(r => pathname === r.path);
        } else if (pathname.startsWith(routes.cashier_p2p) && !!route.routes) {
            const cashier_subroutes = route?.routes.find(r => r.path === routes.cashier_p2p);
