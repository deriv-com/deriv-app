...
const Collapsible = ({ as, is_collapsed, position = "top", children }) => {
  ...
  const arrow_button = (
    <ArrowButton is_open={is_open} position={position} onClick={toggleExpand} />
  );
          ...
          const collapsed_class = classNames(
            ...
            element.props.className,
            {
              "dc-collapsible__item--collapsed":
                "collapsible" in element.props && !is_open
            }
          );

          const no_collapsible_props = { ...element.props };
          if ("collapsible" in no_collapsible_props)
            delete no_collapsible_props.collapsible;
