...
const TwoFactorAuthentication = observer(() => {
    ...
    const generateQrCode = React.useCallback(async () => {
        ...
        const qr_secret_key_value = `otpauth://totp/${client.email_address}?secret=${secret_key_value}&issuer=Deriv.com`;
    ...
    const getDigitStatus = React.useCallback(async () => {
        const status_response = await client.getTwoFAStatus();
        ...
        setLoading(false);
    }, [client.getTwoFAStatus, generateQrCode]);

    if (is_loading || client.is_switching) return <Loading is_fullscreen={false} className='account__initial-loader' />;
    ...
    const TwoFactorEnabled = (
        <ThemedScrollbars is_bypassed={isMobile()} className='two-factor__scrollbars'>
            <div className='two-factor__wrapper--enabled'>
                ...
                <DigitForm
                    is_enabled={client.has_enabled_two_fa}
    ...
    return (
        <section className='two-factor'>
            ...
            >
                {client.has_enabled_two_fa ? TwoFactorEnabled : TwoFactorDisabled}
            </div>
        </section>
    );
});
