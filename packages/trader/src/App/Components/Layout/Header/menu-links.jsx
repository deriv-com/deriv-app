        ...
        <div className='header__menu-links'>
            {
                items.map((item, idx) => (
                    (item.login_only && (item.login_only !== is_logged_in)) ?
                        null
                        :
                        <BinaryLink id={item.id} key={idx} to={item.link_to || undefined} onClick={item.onClick || undefined} className='header__menu-link' active_class='header__menu-link--active'>
                            <React.Fragment>
                                {item.text &&
