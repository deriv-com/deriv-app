const { expect } = require('../../../test_utils/common');
    ...
    urls.forEach(url => {
        describe(url, () => {
            ...
            it('assert mocking globals is working', () => {
                ...
                expect(window.location.href).to.be.eq(Url.websiteUrl());
            });
            describe('.urlFor()', () => {
                ...
                it('handles all valid characters', () => {
                    expect(Url.urlFor('metatrader/comparison-4_vs_5')).to.eq(
            ...
            describe('.paramsHash()', () => {
                it('returns correct object', () => {
                    expect(Url.paramsHash(url_with_qs))
                        .to.be.an('Object')
                ...
                it('returns empty object when there is no query string', () => {
                    expect(Url.paramsHash(url_no_qs))
                        .to.be.an('Object')
                        .and.to.deep.equal({});
                    expect(Url.paramsHash(`${url_no_qs}?`))
                        .to.be.an('Object')
            ...
            if (!/deriv.app/.test(url)) {
                describe('.urlForCurrentDomain()', () => {
                    ...
                    it('updates domain correctly', () => {
                        ...
                        expect(Url.urlForCurrentDomain(`https://www.deriv.app/${path_query_hash}`)).to.eq(
                            `${url}/${path_query_hash}`
                        );
                    });
                    it('updates host maps correctly', () => {
                        ...
                        Object.keys(host_map).forEach(host => {
                            ...
                            expect(Url.urlForCurrentDomain(`https://${host}/${path_query_hash}`)).to.eq(
                    ...
                    it("doesn't update the third party domains", () => {
                        expect(Url.urlForCurrentDomain('https://www.otherdomain.com')).to.eq(
                            'https://www.otherdomain.com'
                        );
                        expect(Url.urlForCurrentDomain('https://www.otherdomain.com/')).to.eq(
                            'https://www.otherdomain.com/'
                        );
                        expect(Url.urlForCurrentDomain('https://subdomain.otherdomain.com/')).to.eq(
                            'https://subdomain.otherdomain.com/'
                        );
                        expect(Url.urlForCurrentDomain('mailto:email@otherdomain.com')).to.eq(
            ...
            describe('.param()', () => {
                ...
                it('returns expected parameter', () => {
                    expect(Url.param('duration_amount'))
                        .to.be.a('string')
                        .and.eq('5');
                    expect(Url.param('no_value'))
                        .to.be.a('string')
                        .and.eq('');
                });
            });
