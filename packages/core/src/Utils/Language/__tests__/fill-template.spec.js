...
describe('fillTemplate', () => {
    ...
    it('works for 2 tags replaced with components', () => {
        ...
        expect(fillTemplate('[_1] think therefore [_1] am', { '1': img })).to.eql([
            img,
            ' think therefore ',
            img,
            ' am',
    ...
    it('works for 2 pair tags replaced with components', () => {
        ...
        expect(result)
            ...
            .to.eql([
                'These ',
                <Link key={result[1].key}>violent delights</Link>,
                ' have ',
                <Link key={result[3].key}>violent ends</Link>,
                '.',
            ]);
    });
    it('works for a mix of tags replaced with strings and components', () => {
        const Link = ({ children }) => <a className='link'>{children}</a>;
        const result = fillTemplate('Please [_1]log in[_2] or [_3]sign up[_2] to view [_4] page.', {
            '1_2': <Link />,
            '3_2': <Link />,
            '4': 'statement',
        });
        expect(result)
            ...
            .to.eql([
                'Please ',
                <Link key={result[1].key}>log in</Link>,
                ' or ',
                <Link key={result[3].key}>sign up</Link>,
                ' to view statement page.',
            ]);
    });
});
