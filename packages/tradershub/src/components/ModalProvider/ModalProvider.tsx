...
const ModalProvider = ({ children }: React.PropsWithChildren<unknown>) => {
    ...
    const show = useCallback(
        (ModalContent: React.ReactNode, options?: TModalOptions) => {
    ...
    useOnClickOutside(modalRef, isLg ? hide : () => undefined);
    ...
    const modalRootRef = useMemo(() => {
        ...
        if (modalOptions?.shouldHideDerivAppHeader || modalOptions?.defaultRootId === 'wallets_modal_root')
            return rootRef;
        // otherwise do the default behavior, show Deriv.app header if on responsive
        if (modalOptions?.defaultRootId === 'wallets_modal_show_header_root' || !isLg) return rootHeaderRef;
        return rootRef;
    }, [modalOptions?.rootRef, modalOptions?.shouldHideDerivAppHeader, modalOptions?.defaultRootId, isLg]);

    // Passing the props directly will cause the modal to re-render on every state change, so we use useMemo to prevent that
