...
const CashierLocked = ({ 
    ...
    const no_residence = cashier_validation && cashier_validation.includes('no_residence');
    const unwelcome_status = cashier_validation && cashier_validation.includes('unwelcome_status');
    const self_exclusion = cashier_validation && cashier_validation.includes('SelfExclusion');
    const no_withdrawal_or_trading_status = cashier_validation && cashier_validation.includes('no_withdrawal_or_trading_status');
    const withdrawal_locked_status = cashier_validation && cashier_validation.includes('withdrawal_locked_status');
    const documents_expired = cashier_validation && cashier_validation.includes('documents_expired');
    const cashier_locked_status = cashier_validation && cashier_validation.includes('cashier_locked_status');
    const disabled_status = cashier_validation && cashier_validation.includes('disabled_status');
    const financial_assessment_required = cashier_validation && cashier_validation.includes('FinancialAssessmentRequired');
    const ask_currency = cashier_validation && cashier_validation.includes('ASK_CURRENCY');
    const ask_authenticate = cashier_validation && cashier_validation.includes('ASK_AUTHENTICATE');
    const ask_financial_risk_approval = cashier_validation && cashier_validation.includes('ASK_FINANCIAL_RISK_APPROVAL');
    const ask_tin_information = cashier_validation && cashier_validation.includes('ASK_TIN_INFORMATION');
    const ask_self_exclusion_max_turnover_set = cashier_validation && cashier_validation.includes('ASK_SELF_EXCLUSION_MAX_TURNOVER_SET');
    const ask_fix_details = cashier_validation && cashier_validation.includes('ASK_FIX_DETAILS');
    const ask_uk_funds_protection = cashier_validation && cashier_validation.includes('ASK_UK_FUNDS_PROTECTION');
    ...
    if (is_system_maintenance) {
        ...
        if (financial_assessment_required) {
            ...
            message = localize('Your cashier is locked. Please complete the financial assessment to unlock it.');
        }
        if (ask_currency) {
