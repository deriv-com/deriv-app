...
class Autocomplete extends React.PureComponent {
  ...
  setActiveUp = () => {
    ...
    if (typeof active_index === "number") {
      ...
      if (should_scroll_to_last) {
        ...
        const item_height = this.list_item_ref.current.getBoundingClientRect()
          .height;
        const item_top =
          Math.floor(this.list_item_ref.current.getBoundingClientRect().top) -
          item_height;

        if (!this.isListItemWithinView(item_top)) {
          const top_of_list =
            this.list_item_ref.current.offsetTop - item_height;
  ...
  isListItemWithinView(item_top) {
    ...
    const wrapper_top = Math.floor(
      this.list_wrapper_ref.current.getBoundingClientRect().top
    );
    const wrapper_bottom =
      Math.floor(this.list_wrapper_ref.current.getBoundingClientRect().top) +
      list_height;
      ...
      if (should_scroll_to_first) {
        ...
        if (!this.isListItemWithinView(item_top)) {
          ...
          const bottom_of_list =
            this.list_item_ref.current.offsetTop - items_above * item_height;
  ...
  onBlur = e => {
    ...
    if (
      ...
      typeof this.props.onItemSelection === "function"
    ) {
    ...
    return (
      <div className={classNames("dc-autocomplete", className)}>
        ...
        <DropdownList
          ...
          style={{
            width: this.input_wrapper_ref
              ? `${this.input_wrapper_ref.offsetWidth}px`
              : "100%",
