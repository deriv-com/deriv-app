...
class Dropdown extends React.Component {
  ...
  get transition_class_names() {
    return {
      enter: `dc-dropdown__list--enter${
        this.props.is_alignment_left ? " dc-dropdown__list--left--enter" : ""
      }`,
  ...
  handleClickOutside = event => {
    if (
      ...
      (!this.wrapper_ref.contains(event.target) &&
        !this.wrapper_ref.contains(event.path[0])) && // event.path[0] is the node that the event originated from, it does not need to walk the array
  ...
  onKeyPressed = event => {
    ...
    const index = this.props.value
      ? getItemFromValue(this.props.list, this.props.value)
      : 0;
    const value = this.props.value
      ? getValueFromIndex(this.props.list, this.state.curr_index)
      : null;
