...
const concatenate = (number, default_value) =>
  default_value.toString().concat(number);
...
const Numpad = ({
        ...
        if (default_value === 0) {
          ...
          if (matches !== null && is_float) {
            matches.forEach((match, groupIndex) => {
              const pip_size_allowed =
                groupIndex === 1 && match.length < pip_size && is_float;
              if (pip_size_allowed) {
                setValue(concatenate(num, default_value));
              }
            });
  ...
  React.useEffect(() => {
    if (is_currency && typeof pip_size === "undefined") {
      ...
      console.error(
        "Warning: property pip_size is required when using currency type <Numpad pip_size='2' />"
      );
    }
  });
