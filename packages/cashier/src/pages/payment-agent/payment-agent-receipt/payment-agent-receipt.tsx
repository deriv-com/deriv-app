...
import { TRootStore, TPaymentAgentDetails } from 'Types';
import './payment-agent-receipt.scss';

type TPaymentAgentDetailsProps = Pick<TPaymentAgentDetails, 'email' | 'phone_numbers' | 'urls'>;

type TOpenStatement = RouteComponentProps & {
    resetPaymentAgent: () => void;
};

type TPaymentAgentReceiptProps = TOpenStatement & {
    currency: string;
    is_from_derivgo: boolean;
    receipt: {
        payment_agent_name: string;
        amount_transferred: number;
        payment_agent_email: TPaymentAgentDetails['email'];
        payment_agent_phone: TPaymentAgentDetails['phone_numbers'];
        payment_agent_url: TPaymentAgentDetails['urls'];
    };
};

const openStatement = (history: string[], resetPaymentAgent: () => void) => {
    history.push(routes.statement);
    resetPaymentAgent();
};

const PaymentAgentDetails = ({ email, phone_numbers, urls }: TPaymentAgentDetailsProps) => {
    return (
