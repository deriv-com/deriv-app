...
export const getNormalizedPaymentMethod = (payment_method: string) => {
...
export const getUniquePaymentAgentSupportedBanks = (supported_banks?: { payment_method: string }[]) => {
    const normalized_payment_methods =
        supported_banks && supported_banks.map(bank => getNormalizedPaymentMethod(bank.payment_method)).filter(Boolean);
