...
jest.mock('../send', () => jest.fn().mockImplementation(() => Promise.resolve({result: 'data received from send', subscription: { id: 'SUBSCRIPTION_ID' }})));
...
describe('BackendSubscription', () => {
    ...
    test('calls listener with initial data', async () => {
        ...
        expect(listenerMock).toHaveBeenCalledWith({result: 'data received from send', subscription: { id: 'SUBSCRIPTION_ID' }});    
    });
    ...
    test('calls all the listeners with initial data', async () => {
        ...
        expect(listenerMock1).toHaveBeenCalledWith({result: 'data received from send', subscription: { id: 'SUBSCRIPTION_ID' }});

        expect(listenerMock2).toHaveBeenCalledWith({result: 'data received from send', subscription: { id: 'SUBSCRIPTION_ID' }});

        expect(listenerMock3).toHaveBeenCalledWith({result: 'data received from send', subscription: { id: 'SUBSCRIPTION_ID' }});
    });
    ...
    test('sends updated data to all the listneres', async () => {
        ...
        const fakeResponse2 = { req_id: backendSubscription.reqId, data: 'test data 2', subscription: { id: 'SUBSCRIPTION_ID' } };
        mockWs.respondFromServer(JSON.stringify(fakeResponse2));

        expect(listenerMock1.mock.calls[0][0]).toEqual({result: 'data received from send', subscription: { id: 'SUBSCRIPTION_ID' }});
        expect(listenerMock1.mock.calls[1][0]).toEqual({ req_id: backendSubscription.reqId, data: 'test data 2', subscription: { id: 'SUBSCRIPTION_ID' } });

        expect(listenerMock2.mock.calls[0][0]).toEqual({result: 'data received from send', subscription: { id: 'SUBSCRIPTION_ID' }});
        expect(listenerMock2.mock.calls[1][0]).toEqual({ req_id: backendSubscription.reqId, data: 'test data 2', subscription: { id: 'SUBSCRIPTION_ID' } });

        expect(listenerMock3.mock.calls[0][0]).toEqual({result: 'data received from send', subscription: { id: 'SUBSCRIPTION_ID' }});
        expect(listenerMock3.mock.calls[1][0]).toEqual({ req_id: backendSubscription.reqId, data: 'test data 2', subscription: { id: 'SUBSCRIPTION_ID' } });        
    });
    ...
    test('ignores messages with different req_id', async () => {
        ...
        const fakeBadResponse = { req_id: +(backendSubscription.reqId || 1) + 3, data: 'test data 2', subscription: { id: 'SUBSCRIPTION_ID' } };
        mockWs.respondFromServer(JSON.stringify(fakeBadResponse));
        
        // Simulate server response
        const fakeCorrectResponse = { req_id: backendSubscription.reqId, data: 'test data 2', subscription: { id: 'SUBSCRIPTION_ID' } };
    ...
    test('when unsubscribing, sends forget request with received subscription id', async () => {
        ...
        mockWs.respondFromServer(JSON.stringify({ req_id: backendSubscription.reqId, data: 'test data 2', subscription: { id: 'SUBSCRIPTION_ID' } }));
        await subscribePromise;

        // // Execute unsubscribe
        await backendSubscription.unsubscribe();

        expect(send).toHaveBeenCalledWith(mockWs, expect.any(Number), 'forget', { forget: 'SUBSCRIPTION_ID' });
    });
