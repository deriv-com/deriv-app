...
import { TAccount, TRootStore, TTransferBetweenAccountsResponse, TWebSocket } from 'Types';

const hasTransferNotAllowedLoginid = (loginid: string | undefined): boolean => !!loginid?.startsWith('MX');
...
export default class AccountTransferStore {
    ...
    @observable minimum_fee: string | null = null;
    @observable receipt = {};
    @observable selected_from: TAccount | undefined = {};
    ...
    @action.bound
    setBalanceByLoginId(loginid: string, balance: string): void {
    ...
    @action.bound
    async sortAccountsTransfer(response_accounts: TTransferBetweenAccountsResponse) {
        ...
        accounts?.forEach(account => {
            ...
            const getCurrencyDisplayCodeText = getCurrencyDisplayCode(
                account.currency !== 'eUSDT' ? account.currency?.toUpperCase() : account.currency
            );
            const account_text_display = is_cfd ? getAccountTextDisplay : getCurrencyDisplayCodeText;
    ...
    requestTransferBetweenAccounts = async ({ amount }: { amount: string }) => {
        const { client, modules } = this.root_store;
