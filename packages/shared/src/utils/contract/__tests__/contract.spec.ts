...
describe('getAccuChartScaleParams', () => {
    const mocked_height_desktop = 700;
    const mocked_height_mobile = 400;
    const {
        max_ticks_mobile,
        scale_settings: { heightFactor },
    } = ContractUtils.getAccuChartScaleParams({ yaxis_height: mocked_height_desktop });
    const desktop_whitespace = ContractUtils.getAccuChartScaleParams({}).scale_settings.whitespace;
    const mobile_whitespace = ContractUtils.getAccuChartScaleParams({ is_mobile: true }).scale_settings.whitespace;
    ...
    it('should return an object with correct scale params for desktop DTrader page', () => {
        expect(ContractUtils.getAccuChartScaleParams({ yaxis_height: mocked_height_desktop })).toEqual({
            ...default_params,
            scale_settings: { ...default_params.scale_settings, whitespace: desktop_whitespace },
            yaxis_margin: {
                top: 189,
                bottom: 189,
            },
    ...
    it('should return an object with correct scale params for mobile DTrader page', () => {
        expect(
            ContractUtils.getAccuChartScaleParams({
                is_mobile: true,
                yaxis_height: mocked_height_mobile,
            })
        ).toEqual({
            ...default_params,
            scale_settings: { ...default_params.scale_settings, whitespace: mobile_whitespace },
            yaxis_margin: {
                top: 132,
                bottom: 132,
            },
    ...
    it('should return an object with correct scale params for desktop C.Details page when tick_passed < or > 5', () => {
        expect(
            ContractUtils.getAccuChartScaleParams({
    ...
    it('should return an object with correct scale params for mobile C.Details page when tick_passed < or > 5', () => {
        expect(
            ContractUtils.getAccuChartScaleParams({
                is_mobile: true,
                yaxis_height: mocked_height_mobile,
