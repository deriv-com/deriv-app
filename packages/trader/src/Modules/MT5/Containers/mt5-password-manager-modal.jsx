...
import { isMobile, validLength, validPassword, getErrorMessages, getStaticUrl, routes } from '@deriv/shared';
...
const MT5PasswordManagerTabContent = ({ status, children }) => {
    const [active_tab_index, updateActiveTabIndex] = React.useState(0);

    return (
        <>
            <Tabs active_index={active_tab_index} onTabItemClick={updateActiveTabIndex} top>
                ...
                <div label={localize('Investor password')}>
                    <DesktopWrapper>
                        <ThemedScrollbars height={password_container_height}>{children}</ThemedScrollbars>
                    </DesktopWrapper>
                    <MobileWrapper>
                        <Div100vhContainer className='mt5-password-manager__scroll-wrapper' height_offset='120px'>
                            {children}
...
const MT5PasswordManagerModal = ({
    ...
    const steps = [
        {
            component: (
                <MT5PasswordManagerTabContent status={account_status.status}>
