...
export const isBot = () => {
    ...
    const pathname = /^\/bot/.test(window.location.pathname);
    const route = /^\/(br_)/.test(window.location.pathname) && window.location.pathname.split('/')[2] === 'bot';
...
export const getPathname = () => {
    const { pathname, route } = isBot();
    if (pathname || route) return platform_name.DBot;
...
export const getPlatformInformation = (routing_history: TRoutingHistory) => {
    const { pathname, route } = isBot();
    if (pathname || route || isNavigationFromPlatform(routing_history, routes.bot)) {
...
export const getActivePlatform = (routing_history: TRoutingHistory) => {
    const { pathname, route } = isBot();
    if (pathname || route || isNavigationFromPlatform(routing_history, routes.bot)) return platform_name.DBot;
