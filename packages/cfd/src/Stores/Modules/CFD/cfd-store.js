...
export default class CFDStore extends BaseStore {
  ...
  get account_title() {
    return this.account_type.category
      ? getMtCompanies(this.root_store.traders_hub.show_eu_related_content)[
  ...
  get current_list() {
    const list = {};
    const show_eu_related_content =
      this.root_store.traders_hub.show_eu_related_content;
  ...
  realAccountSignupEndListener() {
    const post_signup = JSON.parse(
      sessionStorage.getItem("post_real_account_signup")
    );
  ...
  async createCFDAccount({ category, platform, type, set_password }) {
    ...
    if (platform === CFD_PLATFORMS.DXTRADE) {
      ...
      if (!response.error) {
        ...
        const trading_platform_accounts_list_response =
          await WS.tradingPlatformAccountsList(values.platform);
        this.root_store.client.responseTradingPlatformAccountsList(
          trading_platform_accounts_list_response
        );
  ...
  getName() {
    const { first_name } =
      ...
      this.root_store.client.account_settings;
    const title =
      this.mt5_companies[this.account_type.category][this.account_type.type]
        .title;
  ...
  beginRealSignupForMt5() {
    sessionStorage.setItem(
      "post_real_account_signup",
      JSON.stringify(this.account_type)
    );
  ...
  async getAccountStatus(platform) {
    const should_load_account_status =
      (platform === CFD_PLATFORMS.MT5 &&
        this.root_store.client.is_mt5_password_not_set) ||
      (platform === CFD_PLATFORMS.DXTRADE &&
        this.root_store.client.is_dxtrade_password_not_set);
  ...
  async submitMt5Password(values, actions) {
    ...
    if (!response.error) {
      ...
      this.root_store.client.responseMT5TradingServers(
        await WS.tradingServers(CFD_PLATFORMS.MT5)
      );
    ...
    if (!response.error) {
      ...
      switch (platform) {
        ...
        case CFD_PLATFORMS.MT5: {
          await WS.authorized
            .mt5LoginList()
            .then(this.root_store.client.responseMt5LoginList);
  ...
  sendVerifyEmail() {
    return WS.verifyEmail(
      this.root_store.client.email,
  ...
  loadDxtradeTokens() {
    ["demo", "real"].forEach((account_type) => {
      const has_existing_account =
        this.root_store.client.dxtrade_accounts_list.some(
          (account) => account.account_type === account_type
        );

      if (!this.dxtrade_tokens[account_type] && has_existing_account) {
        WS.getServiceToken(CFD_PLATFORMS.DXTRADE, account_type).then(
          (response) => this.setDxtradeToken(response, account_type)
        );
      }
    });
  }
  ...
  loadDerivezTokens() {
    ["demo", "real"].forEach((account_type) => {
      const has_existing_account =
        this.root_store.client.derivez_accounts_list.some(
          (account) => account.account_type === account_type
        );

      if (!this.derivez_tokens[account_type] && has_existing_account) {
        WS.getServiceToken(CFD_PLATFORMS.DERIVEZ, account_type).then(
          (response) => this.setDerivezToken(response, account_type)
        );
      }
    });
  }
