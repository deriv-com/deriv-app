...
// TODO: Remove unused props for purchase button
const IconCheckmark = () => (
...
const Button = ({
    ...
    const classes = classNames('trader-btn', { effect: has_effect }, className);
    const button = (
        <button
            id={id}
            className={classes}
        ...
        >
            { text && !(is_loading || is_submit_success) &&
                <span className={classNames('trader-btn__text', classNameSpan)}>
                    {text}
                </span>
            }
            { is_loading &&
