...
const Instrument: React.FC<TInstrumentProps> = ({ title, pair_data }) => {
    return (
        <div className='trade-type-template__instrument'>
            <Text size='m' weight='bold' styles={{ lineHeight: '36px' }}>
                {title}
            </Text>
            <div className='trade-type-template__instrument-container'>
                {pair_data.map((item, idx) => (
                    <div key={idx}>
                        <Text size='sm' weight='bold'>
                            {item.title}
                        </Text>
                        <div className='trade-type-template__instrument-pair-wrapper'>
