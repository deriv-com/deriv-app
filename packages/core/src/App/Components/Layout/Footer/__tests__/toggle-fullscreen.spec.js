...
describe('ToggleFullScreen', () => {
    ...
    it('should have .ic-fullscreen', () => {
        const wrapper = shallow(<ToggleFullScreen />);
        expect(wrapper.find('.ic-fullscreen').exists()).toBe(true);
    });
    it("should not have .ic-fullscreen--active if it's false in state", () => {
        const wrapper = shallow(<ToggleFullScreen />);
        expect(wrapper.find('.ic-fullscreen--active').exists()).toBe(false);
    });
    it("should contain <Icon icon='IcMaximize' />", () => {
        const wrapper = shallow(<ToggleFullScreen />);
        expect(wrapper.contains(<Icon icon='IcFullScreen' className='footer__icon' />)).toBe(true);
    });
});
