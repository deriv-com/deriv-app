...
export const getExpiryType = (store: any) => {
    ...
    const expiry_is_after_today =
        expiry_type === 'endtime' &&
        (dayjs(expiry_date).isAfter(dayjs(server_time), 'day') || !hasIntradayDurationUnit(duration_units_list));
...
export const resetEndTimeOnVolatilityIndices = (symbol: string, expiry_type: string | null) =>
    /^R_/.test(symbol) && expiry_type === 'endtime' ? dayjs().format('DD MMM YYYY') : null;
