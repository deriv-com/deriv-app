    ...
    ({
        ...
        if (start.visible) {
            draw_path(ctx, {
                ...
                zoom: start.zoom,
                icon: ICONS.ENTRY_SPOT.withColor(
        ...
        if (exit.visible && is_sold) {
            draw_path(ctx, {
                ...
                zoom: exit.zoom,
                icon: ICONS.END.withColor(color, get_color({ status: 'bg', is_dark_theme })),
            });
        }
        ctx.restore();
    }
    ...
    ({
        ...
        if (start && start.visible && barrier) {
            draw_path(ctx, {
                ...
                left: start.left - 1 * scale,
                zoom: start.zoom,
                icon: ICONS.ENTRY_SPOT.withColor(color + opacity),
            });
        }
        // show the profit
        if (show_profit) {
    ...
    ({
        ...
        if (start.visible && (granularity === 0 || !is_sold)) {
            draw_path(ctx, {
                ...
                left: start.left - 1 * scale,
                zoom: start.zoom,
                icon: ICONS.ENTRY_SPOT.withColor(color + opacity, get_color({ status: 'bg', is_dark_theme }) + opacity),
            });
        }
        // remaining ticks
