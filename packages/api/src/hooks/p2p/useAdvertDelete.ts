...
const useAdvertDelete = () => {
    ...
    const modified_data = useMemo(() => {
        if (!data?.p2p_advert_update) return undefined;

        return {
            ...data?.p2p_advert_update,
            /** Indicates if this is block trade advert or not. */
            is_block_trade: Boolean(data?.p2p_advert_update?.block_trade),
            /** The activation status of the advert. */
            is_active: Boolean(data?.p2p_advert_update?.is_active),
            /** Indicates that this advert will appear on the main advert list. */
            is_visible: Boolean(data?.p2p_advert_update?.is_visible),
            /** Indicates that the advert has been deleted. */
            is_deleted: Boolean(data?.p2p_advert_update?.deleted),
        };
    }, [data?.p2p_advert_update]);
