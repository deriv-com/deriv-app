...
const PositionsContent = ({ isClosedTab, positions = [] }: TPositionsContentProps) => {
    ...
    return (
        <div className={`positions-page__${isClosedTab ? 'closed' : 'open'}`}>
            {positions.length ? <></> : <EmptyMessage isClosedTab={isClosedTab} noMatchesFound={noMatchesFound} />}
        </div>
    );
};
