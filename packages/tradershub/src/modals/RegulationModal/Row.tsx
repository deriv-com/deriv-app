...
const Row = ({ attribute, content, id, idx }: TRowProps) => (
    <tr className={qtMerge('min-h-40', idx === 0 && 'bg-brand-pink-light')} key={id}>
        <td
            className={`sticky z-10 align-middle border-solid start-50 py-500 px-8 border-system-light-active-background border-x-1 border-b-1 ${
        ...
        {Object.keys(content).map(rowKey => (
            <td
                className='flex-col justify-center align-middle border-solid whitespace-nowrap py-500 px-8 border-b-1 border-r-1 border-system-light-active-background'
                key={rowKey}
            >
