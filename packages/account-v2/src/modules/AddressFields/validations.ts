...
export const addressDetailValidations = (countryCode: string, isSvg: boolean) => ({
    addressCity: Yup.string()
        ...
        .matches(
            regexChecks.addressDetails.addressCity,
    ...
    addressLine1: Yup.string()
        ...
        .matches(
            regexChecks.addressDetails.addressLine1,
    ...
    addressLine2: Yup.string()
        .max(70, 'Only 70 characters, please.')
        .matches(
            regexChecks.addressDetails.addressLine2,
            `Use only the following special characters: ${addressPermittedSpecialCharactersMessage}`
        )
        .when({
            is: () => isSvg,
    ...
    addressPostcode: Yup.string()
        ...
        .matches(regexChecks.addressDetails.addressPostcode, 'Only letters, numbers, space and hyphen are allowed.')
        .when({
            is: () => countryCode === 'gb',
            then: Yup.string().matches(
                regexChecks.addressDetails.nonJerseyPostcode,
                'Our accounts and services are unavailable for the Jersey postal code.'
            ),
        }),
    addressState: Yup.string()
        .required('State is required')
        .matches(regexChecks.addressDetails.addressState, 'State is not in a proper format'),
});
