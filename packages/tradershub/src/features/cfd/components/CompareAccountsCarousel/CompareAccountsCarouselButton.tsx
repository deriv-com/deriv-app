...
const CFDCompareAccountsCarouselButton = ({ enabled, isNext = false, onClick }: TPrevNextButtonProps) => (
    <IconButton
        className={qtMerge(
            'bg-system-light-primary-background z-10 absolute lg:flex items-center justify-center top-1/2 cursor-pointer w-2000 h-40 rounded-[50%] border-solid-coral-100 border-solid border-1 disbaled:opacity-8 disabled:hidden',
            isNext && 'right-16',
            !isNext && 'left-16'
        )}
