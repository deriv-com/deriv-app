...
function buildIcons() {
  ...
  const emitModule = file => {
    const pathname = entries_object[file]
      .replace(".svg", "")
      .replace("./src/components/icon/", "");
    ...
    const icon_name = getPascalCase(
      pathname.match(/([^/]*)\/*$/)[1].replace(".svg", "")
    );
  ...
  fs.writeFileSync(
    path.join(__dirname, "../src/components/icon/icons.js"),
    buffer.join(EOL) + EOL
  );

  console.info(
    `Build icons done. 'components/icon/icons.js' emitted with ${modules} modules.`
  );
}
