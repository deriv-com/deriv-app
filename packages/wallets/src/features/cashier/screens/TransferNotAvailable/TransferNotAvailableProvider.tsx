...
const getMessage = ({
    ...
    if (!hasAccountsForTransfer) {
        ...
        return {
            actionButton: () => (
                <WalletButton onClick={() => history.push('/appstore/traders-hub')} size='lg'>
    ...
    if (!hasTransferAccountsWithFunds) {
        ...
        const locationPathName = `/appstore/traders-hub/cashier/${isVirtual ? 'reset-balance' : 'deposit'}` as const;
