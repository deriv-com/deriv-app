...
export const useFilterTutorialsTab = (search, active_tab, active_tab_tutorials) => {
    ...
    const [tutorials_content, setTutorialTabContent] = React.useState([...guide_tab_content, ...faq_tab_content]);
    ...
    React.useEffect(() => {
        const unified_filtered_list = tutorials_content?.filter(({ title, description = [], content }) => {
        ...
        if (!search) {
            ...
            setTutorialTabContent(tutorials_content);
        }
