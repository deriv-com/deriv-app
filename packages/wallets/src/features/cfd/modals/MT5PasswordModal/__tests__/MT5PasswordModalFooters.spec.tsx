...
describe('SuccessModalFooter', () => {
    ...
    it('calls hide when OK button is clicked for demo account', async () => {
    ...
    it('navigates to transfer page when transfer funds button is clicked', async () => {
        const mockHide = jest.fn();
        const mockHistory = { push: jest.fn() };
        (useDevice as jest.Mock).mockReturnValue({ isDesktop: true });
        (useModal as jest.Mock).mockReturnValue({ hide: mockHide });
        ...
        render(<SuccessModalFooter isDemo={false} />);
        await userEvent.click(screen.getByText('Transfer funds'));
        expect(mockHide).toHaveBeenCalled();
